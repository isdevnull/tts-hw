...
class Vocoder(nn.Module):
    ...
    def __init__(self):
        ...
        waveglow_path = Path("./pretrained/waveglow_256channels_universal_v5.pt")
        if not waveglow_path.exists():
            waveglow_path.parent.mkdir(parents=True, exist_ok=True)
            gdd.download_file_from_google_drive(
                file_id='1rpK8CzAAirq9sWZhe9nlfvxMF1dRgFbF',
                dest_path=waveglow_path
            )
        model = torch.load(waveglow_path, map_location='cpu')[
            'model']
